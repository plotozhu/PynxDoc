...
上述的第一条原则保证了数据的最终一致性，第二条原则保证了快速性。 在使用过程中，如果区块哈希获得的结果还是哈希，需要再次获取一次时，可以直接使用原则1，而不必要通过对则2，因此在实现时，可以提供两种不同的接口或是不同的参数。参考[这里](../3-网络层组件/2-链外存储.md)了解更多的关于链外存储的设计和实现
...
## 跨分片交易的结果
如前言所述，跨分片交易结果可能会有两种：
1. 原始交易的结果，其执行的结果通常是 CsaActionItem:`(AccountId,action)`，其中的AccountId是帐户，而Action是需要执行的动作
2. 区块交易的结果，其执行的结果将是 CsaResultItem:`Vec<(ChainRole,Hash)>`，这是一个数组，其中每一项中ChainRole是目标链，Hash是所有的一组需要该链执行的结果Hash值，为什么叫ChainRol而不是ChainId？因为substrate占了（chain identifier)
* 是不是可能会有`<ChainRole,action>`的形式?在智能合约里可能会有，在第一版的时候不考虑它。
...
2. 第二种方式需要更新DispatchOutcome： 
    ...
    2. 分类打包后，数据本地存储，生成哈希(CID),在链外存储系统中存储<CID，哈希>对。
由于在substrate中，原有的Call的dispatch方法就有可能返回一个值，这样就需要修改原有的处理方法，因此改动会比较大，因此采用第一种方案：

1. 在System模块中添加两个存储项目，分别存储所有的跨分片交易的信息CsaActionItem和CsaResultItem
2. 提供保留函数deposit_csa_action_item，和可以将一个CsaItem保存到存储项目中
2. 提供保留函数deposit_csa_result_item，和可以将一个CsaResultItem保存到存储项目中
3. Applyable函数需要生成跨分片交易时，通过deposit_csa_action_item或是deposit_csa_result_item把需要跨分片处理的结果存入到存储项目中
4. Executive在on_finalize时，处理CsaActionItem和CsaResultItem

** 参考[这里](./3.2-跨分片交易数据结构.md#proc)了解更多关于区块交易的详细说明 ** 


## <span id="proc">跨分片交易</span>
...
### 原始交易处理
原始交易总是可以被分成两个部分，需要跨分片交易的部分和不需跨分片交易的部分。当需要跨分片交易时，生成CsaActionItem，然后通过deposit_csa_action_item放置到存储区中。
### 区块交易处理
#### 分片链处理区块交易
分片链总是处理来自于对应中继链的处理交易：
1. 从中继的downstream中可以得到给自身的(ChaiRole,Hash)对
2. 从链外存储中获取Hash对应的Vec<CsaActionItem>
3. 把通过deposit_csa_action_item存储需要本分片处理的跨分片交易

#### 中继链处理区块交易
中继链需要处理来自于下属分片的区块交易和信标链的区块交易
##### 分片链区块交易处理
1. 分片区块中的upstream中的每一条，通过deposit_csa_result_item插入到存储中

##### 信标链区块交易
1. 从信标区块中的downstream得到需要自己处理的(ChainRole,Hash)对
2. 根据哈希从链外存储中获取对应的`Vec<Hash>`
3. 把数组中的每一个哈希，生成(ChainRole,Hash)的CsaResultItem，用deposit_csa_result_item存储到数据区
** 此片不展开成CsaActionItem，因分片给中继的也是CsaResultItem，因此让中继对跨分片交易也是生成CsaResultItem结果，在处理结果时统一处理** 

#### 信标链处理区块交易
信标链处理中继的区块交易
1. 读取每个中继分片中的upstream中的每一条，通过deposit_csa_result_item存储到数据区


## 跨分片交易结果的处理
对于不同类型的链，处理跨分片交易结果csa的方案是不一样的：
### 分片链的处理过程
分片链可能会有两种数据要处理，第一种是区块中各个原始交易所产生的CsaActionItem，第二种是中继链的区块交易后的处理结果，

1. 对于中继链的区块交易的结果：
	1. 中继链执行后，生成了所有需要本分片执行的ActionItem,这些ActionItem会被2自动处理，不需要另外的处理
2. 对于CsaActionItem的处理：根据当前的分片策略，
	1. 把需要本分片处理的ActionItem处理完毕，更新state
	2. 把所有需要跨分片交易的ActionItem依照目标中继分组排序
	3. 按照分组排序的结果分别编码，把原始数据存入到链外存储中，得到CsaResultItem:(ChainRole,Hash)
	4. 把得到的Vec<CsaResultItem>排序后放入到CsaResults对象的upstream中	

### 中继链的处理过程
1. 对分片链区块交易的结果是一堆CsaResultItem，其中有给本中继的，也有给其他中继的
2. 对信标链区块交易的结果是一堆CsaResultItem，都是给本中继的
3. 对于给其他中继的，
	1. 重新按照目标的ChainRole进行分组
	2. 把分组的结果编码，存入到链外存储中，并且得到哈希，
	3. 按照所有的目标生成了`Vec<(ChainRole,Hash)>`组，把这个存入到upstream中
4. 对于给自身的
	1. 根据`CsaResultItem`中的哈希，从链外存储中得到相应的`CsaActionItem`信息
	2. 根据分片指数和`CsaActionItem`中的`AccountId`信息，将所有的`CsaActionItem`按照目标的分片链进行组
	3. 将分组结果存入到链外存储中，得到哈希，从而生成新一组的关于对应分片的`Vec<(ChainRole,Hash)>`,把它存入到downstream中  

** 由于分片给中继的也是CsaResultItem，因此让中继对跨分片交易也是生成CsaResultItem结果，在处理结果时统一处理** 

### 信标链的处理过程
1. 对中继链的区块交易处理后，得到一组CsaResultItem:
	1. 根据分片指数和CsaResultItem中的ChainRole,重新将所有的CsaResultItem分组
	2. 将分组的结果编码存入到链外存储中，并且得到哈希，从而生成一组新的`Vec<(ChainRole,Hash)>`，把它存入到downstream中
